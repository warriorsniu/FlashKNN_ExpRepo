...
namespace cukd {
  ...
  struct HeapCandidateList : public CandidateList<k>
  {
    using CandidateList<k>::entry;
    using CandidateList<k>::encode;
    // Historical FlashKNN/cuKD integration allocates a power-of-two
    // compile-time capacity while using an arbitrary runtime k (notably 24
    // and 48). Only the first num_nbr entries participate in the heap.
    int num_nbr = k;
    
    // ------------------------------------------------------------------
    // interface fcts with which _user_ can read results of query:
    // ------------------------------------------------------------------
    inline __host__ __device__ HeapCandidateList(float cutOffRadius, int num_nbr = k);
    inline __host__ __device__ float maxRadius2() const;
    // ------------------------------------------------------------------
    // interface for traversal/query routines to interact with this
    // ------------------------------------------------------------------
...
namespace cukd {
  ...
  template<int k>
  inline __host__ __device__
  HeapCandidateList<k>::HeapCandidateList(float cutOffRadius, int num_nbr)
    : CandidateList<k>(cutOffRadius)
  {
    this->num_nbr = num_nbr;
#pragma unroll
    for (int i=0;i<k;i++)
      entry[i] = encode(cutOffRadius*cutOffRadius,-1);
  }
                                  ...
                                  int pointID)
  {
    ...
    while (true) {
      uint64_t largestChildValue = uint64_t(-1);
      int firstChild = 2*pos+1;
      int largestChild = num_nbr;
      if (firstChild < num_nbr) {
        largestChild = firstChild;
        largestChildValue = entry[firstChild];
      }
        
      int secondChild = firstChild+1;
      if (secondChild < num_nbr && entry[secondChild] > largestChildValue) {
        largestChild = secondChild;
        largestChildValue = entry[secondChild];
      }

      if (largestChild == num_nbr || largestChildValue < e) {
        entry[pos] = e;
        break;
      } else {
        entry[pos] = largestChildValue;
