...
PYTHONPATH="$REPO_DIR/FlashKNN" python "$REPO_DIR/DeLA/S3DIS/benchmark_latency.py" \
...
for MODEL in $NETWORK_MODELS; do
  POINTCEPT_ARGS=("$REPO_DIR/Pointcept/benchmark_latency.py" --model "$MODEL"
    ...
    --warmups "$WARMUPS" --repeats "$REPEATS" --max-samples "$SAMPLES")
  PYTHONPATH="$REPO_DIR/Pointcept" python "${POINTCEPT_ARGS[@]}"
done
...
if [[ -n "${EXPREPO_SEMANTICKITTI:-}" ]]; then
  for MODEL in dela deepla; do
    ...
    PYTHONPATH="$REPO_DIR/FlashKNN:$REPO_DIR/Networks" python "$REPO_DIR/Networks/benchmark_semantickitti_network.py" \
