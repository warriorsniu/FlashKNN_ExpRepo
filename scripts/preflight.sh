...
python "$REPO_DIR/scripts/preflight.py" "${PREFLIGHT_ARGS[@]}" "$@"

if [[ " $* " != *" --data-only "* ]]; then
  PYTHONPATH="$REPO_DIR/Pointcept" python -c \
  ...
  if [[ "${SMOKE:-0}" != "1" ]]; then
    python "$REPO_DIR/scripts/verify_network_models.py" --s3dis "$EXPREPO_S3DIS"
  fi
fi
