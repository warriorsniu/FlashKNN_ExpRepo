...
| `l20_supplement_gpu1_20260807/` | 第二张 L20 上补齐旧版 SemanticKITTI query、S3DIS 网络结果和早期仅 FlashKNN hierarchy 的 DeLA/DeepLA LiDAR 网络结果。`semantickitti.concurrent-invalid.json` 是早期与 CPU nanoflann 并发运行时保存的424条中间记录。 | 旧版 SemanticKITTI query 仅覆盖k=16/24/32且 IVF 匹配 alpha=8，已被2026-08-18的六k全量取代；5个 S3DIS 网络 JSON 已汇入最终目录；早期2个 LiDAR 网络 JSON 已被后续成对结果取代。`concurrent-invalid` 明确无效。 |
...
| `l20_semantickitti_six_k_smoke_20260818/` | 正式补跑前使用1帧、pre/post、六个k、1次 warmup和1次记录验证全部方法与 IVF alpha=4 校准，共12条。 | **已被正式1320条全量完全覆盖。** 仅用于调用链检查，不用于性能结论。 |
