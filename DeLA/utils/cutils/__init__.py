...
def _extension_fingerprint():
    ...
    runtime = (
        ...
        "-O3 -mavx2 -funroll-loops -Xptxas -v",
    )
    digest.update(repr(runtime).encode())
    for source in sorted((path / "srcs").iterdir()):
        if source.is_file():
...
def _load_binary(binary):
    spec = importlib.util.spec_from_file_location("cutils_", binary)
    if spec is None or spec.loader is None:
        raise ImportError(f"Cannot load cached extension: {binary}")
...
try:
    import dela_cutils_ext as cutils
except ImportError:
    # Development fallback for users who import the source tree without first
    # running scripts/install.sh. Release experiments use the installed module
    # above and never enter the JIT build path.
    fingerprint = _extension_fingerprint()
    binary = build_dir / "cutils_.so"
    stamp = build_dir / "cutils_.fingerprint"
    if binary.is_file() and stamp.is_file() and stamp.read_text().strip() == fingerprint:
        cutils = _load_binary(binary)
    else:
        ...
        temporary_stamp = stamp.with_suffix(".tmp")
